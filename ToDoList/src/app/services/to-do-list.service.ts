...
export class ToDoListService {
  ...
  unioneArrayToDoConArrayUtenti(utenti: Iusers[]): void {
    this.toDoArraySvc = this.toDoArraySvc.map((toDo) => {
      const UtenteTrovato = utenti.find((utente) => utente.id === toDo.userId);

      if (!UtenteTrovato) return toDo;

      return {
        ...toDo,
        utenteArray: UtenteTrovato,
      };
    });
  }

  filtroCompleti(array: ItoDo[]) {
    return array.filter((a) => a.completed === true);
  }
}
