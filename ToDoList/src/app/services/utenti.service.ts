...
export class UtentiService {
  ...
  UtentiConTask: (Iusers & { tasks: ItoDo[] })[] = [];

  unioneDelleTaskAgliUtenti(tasks: ItoDo[]) {
    this.UtentiConTask = this.utentiArraySvc.map((utente) => {
      const userTasks = tasks.filter((t) => t.userId === utente.id);
      return {
        ...utente,
        tasks: userTasks,
      };
